...
import policyPendingStatusMessage from '@salesforce/label/c.ABHI_Kavach_Pending';
import policyNoDataAvailableStatusMessage from '@salesforce/label/c.ABHI_Kavach_No_data_Available';
...
export default class Abhi_kavach_Policies extends LightningModal {
    ...
    label = {
        ...
        policyPendingStatusMessage,
        policyNoDataAvailableStatusMessage
    };
        ...
        if(this.searchPolicy) {
            ...
            getPolicyData({policyNo: this.searchPolicy}).then(result=>{
                ...
                if(this.data.length > 0 && !this.data[0].hasOwnProperty('status')) {
                    ...
                    if(this.data[0].message.includes('Waiting for API response')) {
                        this.modalBody = policyPendingStatusMessage;
                    }
                    else {
                    ...
                    //Show popup
                    this.isModalOpen = true;
                    console.log('@@data1'+this.data[0].message);
                    if(this.data[0].message.includes('No data available')) {
                        this.modalBody = policyNoDataAvailableStatusMessage+' '+policyCreationStatusMessage;
                    }
                    else {
                        this.modalBody = this.data[0].message+'\n'+policyCreationStatusMessage;
                    }
